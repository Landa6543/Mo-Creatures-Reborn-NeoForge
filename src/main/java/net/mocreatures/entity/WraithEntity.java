...
public class WraithEntity extends Monster {
   ...
   protected void registerGoals() {
      ...
      this.goalSelector.addGoal(3, new RandomStrollGoal(this, 0.8, 20) {
         protected Vec3 getPosition() {
            RandomSource random = WraithEntity.this.getRandom();
            double dir_x = WraithEntity.this.getX() + (random.nextFloat() * 2.0F - 1.0F) * 16.0F;
            double dir_y = WraithEntity.this.getY() + (random.nextFloat() * 2.0F - 1.0F) * 16.0F;
            double dir_z = WraithEntity.this.getZ() + (random.nextFloat() * 2.0F - 1.0F) * 16.0F;
   ...
   public static void init(RegisterSpawnPlacementsEvent event) {
      event.register(
         MocreaturesModEntities.WRAITH.get(),
