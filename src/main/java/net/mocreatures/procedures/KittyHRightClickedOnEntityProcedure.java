...
public class KittyHRightClickedOnEntityProcedure {
   public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
      if (entity != null && sourceentity != null) {
         if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == Items.COOKED_COD) {
            if (entity instanceof KittyHEntity _datEntSetL) {
               _datEntSetL.getEntityData().set(KittyHEntity.DATA_ReadyToTame, true);
            }
         ...
         if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == MocreaturesModItems.MEDALLION.get()
            && entity instanceof KittyHEntity _datEntL10
            && (Boolean)_datEntL10.getEntityData().get(KittyHEntity.DATA_ReadyToTame)) {
            if (sourceentity instanceof Player _player) {
            ...
            if (sourceentity instanceof ServerPlayer _ent) {
               ...
               _ent.openMenu(new MenuProvider() {
                  public Component getDisplayName() {
                     return Component.literal("NamingKittyH");
                  }
                  ...
                  public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
                     return new NamingKittyHMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(_bpos));
                  }
               }, _bpos);
            }
